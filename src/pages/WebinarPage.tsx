...
const DARK_BG = THEME_DARK_BG;
const GOLD = THEME_GOLD_GRD;
...
function DarkTile({ v, label }: { v: number; label: string }) {
  return (
    <div className="flex flex-col items-center gap-1">
      <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.06] font-cinzel text-2xl font-bold sm:h-20 sm:w-20 sm:text-3xl">
        <span className="text-gradient-gold">{String(v).padStart(2, '0')}</span>
      </div>
      <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/90">{label}</span>
    </div>
  );
}
...
function ImgCard({ src, title, desc, accent, delay }: { src: string; title: string; desc: string; accent: string; delay: number }) {
  return (
    <motion.div
      ...
      className="group relative cursor-default overflow-hidden rounded-3xl border border-white/[0.08] bg-white/[0.03] transition-colors duration-300 hover:border-gold-400/25"
    >
      <div className="relative h-52 overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-nebula-900/95 via-nebula-900/35 to-transparent" />
        <div
          className="absolute inset-x-0 top-0 h-0.5"
          style={{ background: `linear-gradient(90deg,transparent,${accent},transparent)` }}
        />
      </div>
      <div className="border-t border-white/[0.06] bg-nebula-900/90 p-5">
        <h3 className="mb-1.5 font-cinzel text-sm font-bold uppercase tracking-wide text-white">{title}</h3>
        <p className="text-xs leading-relaxed text-white/90">{desc}</p>
      </div>
    </motion.div>
  );
}
      ...
      {(['name', 'email', 'phone'] as const).map((f) => (
        <div key={f}>
          ...
          <input
            ...
            onFocus={(e) => (e.currentTarget.style.borderColor = 'rgba(216,138,34,0.55)')}
            onBlur={(e) => (e.currentTarget.style.borderColor = dark ? 'rgba(255,255,255,0.1)' : 'rgba(229,231,235,1)')}
          />
        </div>
      ))}
      <AnimatePresence>
...
export function WebinarPage() {
  ...
  return (
    <div>
      ...
      {/* ── Gold ribbon ───────────────────────────────────────────── */}
      <div className="px-4 py-2 text-center text-[10px] font-bold uppercase tracking-[0.22em] text-nebula-900" style={{ background: GOLD }}>
      ...
      <section className="relative min-h-[92vh] flex flex-col items-center justify-center overflow-hidden" style={{ background: DARK_BG }}>
        ...
        <div className="pointer-events-none absolute top-0 right-0 h-[400px] w-[400px] rounded-full bg-[radial-gradient(circle,rgba(0,94,168,0.12),transparent_65%)] blur-3xl" />

        <div className="relative z-10 mx-auto max-w-4xl space-y-8 px-4 text-center">
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
            className="inline-flex items-center gap-2.5 rounded-full border border-gold-400/35 bg-gold-400/10 px-5 py-2 text-[10px] font-bold uppercase tracking-[0.25em] text-gold-300">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.25 }} className="flex flex-wrap justify-center gap-3">
            {[{ icon: Calendar, t: '28 June 2025, Saturday' }, { icon: Clock, t: '11:00 AM – 12:20 PM IST' }, { icon: Users, t: 'Free · 80 minutes' }].map(({ icon: Icon, t }) => (
              <div key={t} className="flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.07] px-4 py-2 text-xs font-medium text-white/90">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.35 }}>
            <p className="mb-3 text-[9px] font-bold uppercase tracking-[0.25em] text-white/90">Starts in</p>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.45 }}>
            ...
            <p className="mt-3 text-[10px] uppercase tracking-widest text-white/90">No credit card · Free replay</p>
          </motion.div>
        </div>
      ...
      {/* ══════════════════════════════════════════════════════════════
          STATS STRIP — LIGHT
      ══════════════════════════════════════════════════════════════ */}
      <div className="border-y border-white/[0.06] py-12" style={{ background: THEME_DARK_STRIP_BG }}>
        <div className="mx-auto grid max-w-4xl grid-cols-2 gap-6 px-4 text-center sm:grid-cols-4">
          {[{ v: '25+', l: 'Years of Practice' }, { v: '1.2L', l: 'Clients Guided' }, { v: '50+', l: 'Countries' }, { v: 'Free', l: 'No Cost to Attend' }].map((s) => (
            <div key={s.l}>
              <div className="font-cinzel text-2xl font-bold text-gradient-gold md:text-3xl">{s.v}</div>
              <div className="mt-1 text-[10px] font-semibold uppercase tracking-widest text-white/90">{s.l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <section className="relative overflow-hidden py-20 md:py-28" style={{ background: DARK_BG }}>
        ...
        <div className="pointer-events-none absolute -bottom-20 -right-20 h-[400px] w-[400px] rounded-full bg-[radial-gradient(circle,rgba(0,94,168,0.12),transparent_65%)] blur-3xl" />
        <div className="relative mx-auto max-w-7xl px-4 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.7 }} className="mx-auto mb-14 max-w-xl text-center">
            <span className="mb-3 inline-block text-[10px] font-bold uppercase tracking-[0.25em] text-gold-300">What You&apos;ll Learn</span>
          ...
          <div className="grid sm:grid-cols-3 gap-5 mb-5">
            ...
            <ImgCard src={IMG.cosmos}  title="Decode Your Dasha"      desc="How planetary periods control the timing of major events."        accent="#3387D3" delay={0.07} />
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            ...
            <ImgCard src={IMG.milky}   title="Career Clarity"         desc="Which periods favour moves, promotions, and new ventures."        accent="#D88A22" delay={0.28} />
            <ImgCard src={IMG.stars}   title="Live Q&A"               desc="Ask your chart question in the final open session."               accent="#005EA8" delay={0.35} />
          </div>
        </div>
      </section>
          ...
          <div className="space-y-3">
            {[
              ...
              <motion.div key={item.time}
                ...
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-gold-400/20 bg-gold-400/10 text-lg text-gold-400" style={{ fontFamily: '"Cormorant Garamond","Apple Symbols","Segoe UI Symbol",serif' }}>
                  {item.glyph}
                </div>
      ...
      <section className="py-16 md:py-20 overflow-hidden relative" style={{ background: DARK_BG }}>
        ...
        <div className="relative max-w-5xl mx-auto px-4 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
            className="grid overflow-hidden rounded-3xl border border-gold-400/15 bg-white/[0.03] lg:grid-cols-[2fr_3fr]">
            {/* Portrait */}
            <div className="relative min-h-[320px] overflow-hidden lg:min-h-0">
              <img src={GURU_IMG_RESOURCE} alt="Gurudev Anand — host" className="absolute inset-0 h-full w-full object-cover object-top" />
              <div className="absolute inset-0 bg-gradient-to-b from-transparent to-nebula-900/80 lg:bg-gradient-to-r lg:from-transparent lg:to-nebula-900/80" />
            </div>
            {/* Text */}
            <div className="flex flex-col justify-center px-8 py-10 lg:px-10">
              <span className="mb-3 inline-block text-[10px] font-bold uppercase tracking-[0.25em] text-gold-300">Your Host</span>
              ...
              <p className="mb-7 max-w-md text-sm leading-relaxed text-white/90">
              ...
              <div className="flex flex-wrap gap-3 mb-7">
                {[{ g: '♄', v: '25+', l: 'Years' }, { g: '☉', v: '1.2L+', l: 'Clients' }, { g: '♃', v: '50+', l: 'Countries' }].map((s) => (
                  <div key={s.l} className="flex items-center gap-2 rounded-xl border border-gold-400/20 bg-gold-400/[0.07] px-4 py-2.5">
                    <span className="text-base text-gold-300" style={{ fontFamily: '"Cormorant Garamond","Apple Symbols","Segoe UI Symbol",serif' }}>{s.g}</span>
                    <div>
                      <div className="font-cinzel text-sm font-bold leading-none text-gradient-gold">{s.v}</div>
                      <div className="mt-0.5 text-[9px] uppercase tracking-wide text-white/90">{s.l}</div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className="ml-1 text-xs text-white/90">4.9 / 5 · 10,000+ reviews</span>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      {/* ══════════════════════════════════════════════════════════════
          TESTIMONIALS — LIGHT
      ══════════════════════════════════════════════════════════════ */}
      <section className="relative overflow-hidden py-20 md:py-24" style={{ background: DARK_BG }}>
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute top-0 left-1/2 h-[400px] w-[600px] -translate-x-1/2 rounded-full bg-[radial-gradient(ellipse,rgba(0,94,168,0.1),transparent_70%)] blur-3xl" />
        </div>
        <div className="relative mx-auto max-w-5xl px-4 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.7 }} className="mb-12 text-center">
            <span className="mb-3 inline-block text-[10px] font-bold uppercase tracking-[0.25em] text-gold-300">Past Attendees</span>
            <TextReveal as="h2" className="font-serif text-3xl font-bold leading-tight text-white md:text-4xl">
              What people{' '}
              <span className="italic font-light text-gradient-gold">took home</span>
            </TextReveal>
          </motion.div>
          <div className="grid gap-5 md:grid-cols-3">
            {[
              ...
              <motion.div key={t.name}
                ...
                className="relative flex flex-col overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.05] p-6 backdrop-blur-sm">
                <div className="mb-4 flex gap-0.5">
                  {Array.from({ length: t.stars }).map((_, j) => <Star key={j} size={12} className="text-gold-400 fill-gold-400" />)}
                </div>
                <p className="mb-5 flex-1 text-sm leading-relaxed text-white/90">&ldquo;{t.text}&rdquo;</p>
                <div className="flex items-center gap-3">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full border border-gold-400/25 bg-gold-400/15 font-cinzel text-xs font-bold text-gold-300">
                    {t.name[0]}
                  </div>
                  <div>
                    <p className="text-sm font-semibold leading-none text-white">{t.name}</p>
                    <p className="mt-0.5 text-[10px] text-white/90">{t.loc}</p>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <section className="py-20 md:py-24 relative overflow-hidden" style={{ background: DARK_BG }}>
        ...
        <div className="relative max-w-lg mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.7 }} className="text-center mb-10">
            <span className="mb-3 inline-block text-[10px] font-bold uppercase tracking-[0.25em] text-gold-300">Join Us Live</span>
            ...
            <p className="mt-3 text-sm text-white/90">28 June · 11 AM IST · Free · 80 minutes</p>
          </motion.div>
          <motion.div ref={registerRef}
            ...
            className="rounded-2xl border border-gold-400/20 bg-white/[0.04] p-7 backdrop-blur-md">
            <RegisterForm dark />
          </motion.div>
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }}
            className="mt-6 flex flex-col items-center justify-center gap-4 text-xs text-white/90 sm:flex-row">
      ...
      {/* ══════════════════════════════════════════════════════════════
          FAQ — LIGHT
      ══════════════════════════════════════════════════════════════ */}
      <section className="relative overflow-hidden py-16 md:py-24" style={{ background: DARK_BG }}>
        <div className="pointer-events-none absolute bottom-0 left-1/2 h-[400px] w-[600px] -translate-x-1/2 rounded-full bg-[radial-gradient(ellipse,rgba(0,94,168,0.1),transparent_70%)] blur-3xl" />
        <div className="relative mx-auto max-w-2xl px-4">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.7 }} className="mb-10 text-center">
            <span className="mb-3 inline-block text-[10px] font-bold uppercase tracking-[0.25em] text-gold-300">FAQs</span>
            <TextReveal as="h2" className="font-serif text-2xl font-bold leading-tight text-white md:text-3xl">
              Common{' '}
              <span className="italic font-light text-gradient-gold">questions</span>
            </TextReveal>
          </motion.div>
          <div className="space-y-2">
            {[
              ...
              <motion.div key={item.q}
                ...
                className={`overflow-hidden rounded-2xl border transition-colors duration-300 ${faqOpen === i ? 'border-gold-400/30 bg-gold-400/[0.04]' : 'border-white/[0.08] bg-white/[0.03] hover:border-white/[0.14]'}`}>
                <button type="button" onClick={() => setFaqOpen(faqOpen === i ? null : i)}
                  className="flex w-full items-center gap-4 px-5 py-4 text-left">
                  <span className="flex-1 text-sm font-semibold text-white/95">{item.q}</span>
                  <motion.div animate={{ rotate: faqOpen === i ? 180 : 0 }} transition={{ duration: 0.25 }}>
                    <ChevronDown size={15} className={faqOpen === i ? 'text-gold-400' : 'text-white/80'} />
                  </motion.div>
                </button>
                <AnimatePresence initial={false}>
                  {faqOpen === i && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                      ...
                      <p className="border-t border-white/[0.06] px-5 pb-4 pt-3 text-sm leading-relaxed text-white/90">{item.a}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
          ...
          <motion.div initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 80, opacity: 0 }}
            ...
            <div className="sticky-cta-bar pointer-events-auto">
              <div className="min-w-0 flex-1">
                <p className="truncate text-[11px] font-semibold leading-tight text-white sm:text-xs">Free Live Webinar</p>
                <p className="truncate text-[9px] text-white/90 sm:text-[10px]">28 June · 11 AM IST · 80 min</p>
              </div>
              <button type="button" onClick={scrollToReg}
